...
class ModelMiniBatch:
    ...
    def train(self, input_batch, target_batch, encoder_optimizer, decoder_optimizer, criterion, max_length=20):
        loss = 0
        batch_size = len(input_batch)

        for j in range(batch_size):
            input_variable = input_batch[j]
            target_variable = target_batch[j]
    ...
    def trainIters(self, pairs, n_iters, print_every=100, plot_every=1000, learning_rate=0.01, batch_size=10):
        start = time.time()
        plot_losses = []
        ...
        training_pairs = [self.variablesFromPair(random.choice(pairs)) for i in range(n_iters*batch_size)]
        inputs, targets = zip(*training_pairs)
        criterion = nn.NLLLoss()

        # now proceed one iteration at a time
        for iter in range(1, n_iters + 1):
            i_start = batch_size * (iter-1)
            i_end = i_start+batch_size

            input_batch = inputs[i_start:i_end]
            target_batch = targets[i_start:i_end]

            # train on one example
            loss = self.train(input_batch, target_batch, encoder_optimizer, decoder_optimizer, criterion)
